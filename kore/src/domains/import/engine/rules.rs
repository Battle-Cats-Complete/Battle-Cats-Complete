...
use crate::common::io;
use crate::domains::settings::{ExceptionList, ExceptionRule};
...
pub fn compile() -> (RegexSet, Vec<ExceptionRule>) {
    ...
    for rule in exceptions.rules {
        if rule.pattern.is_empty() && rule.extension.is_empty() {
            continue;
        }

        let extension_string = if rule.extension.is_empty() {
            String::new()
        } else {
            format!(r"\.(?:{})", rule.extension)
        };
