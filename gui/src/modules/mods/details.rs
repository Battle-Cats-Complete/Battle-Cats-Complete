...
pub fn render(ui: &mut egui::Ui, state: &mut ModListState, _settings: &mut Settings) {
    ...
    let Some(mod_idx) = state.data.loaded_mods.iter().position(|m| m.folder_name == selected_id) else { return; };
    ...
    if last_viewed != selected_id {
        state.data.rename_buffer = selected_id.clone();
    ...
    ui.scope(|ui| {
        ...
        let header_response = ui.add(
            egui::TextEdit::singleline(&mut state.data.rename_buffer)
        ...
        if header_response.lost_focus() && state.data.rename_buffer != state.data.loaded_mods[mod_idx].folder_name {
            let old_name = state.data.loaded_mods[mod_idx].folder_name.clone();
            let new_name = state.data.rename_buffer.clone();
            ...
            if !new_name.is_empty() {
                ...
                if !new_path.exists() && old_path.exists() && std::fs::rename(&old_path, &new_path).is_ok() {
                    ...
                    state.data.loaded_mods[mod_idx].metadata.title = new_name.clone();
                    let _ = state.data.loaded_mods[mod_idx].metadata.save(&new_path);
                } else {
                    state.data.rename_buffer = old_name;
                }
            } else {
                state.data.rename_buffer = old_name;
            }
    ...
    let mod_folder = state.data.loaded_mods[mod_idx].folder_name.clone();
    let mod_path = Path::new("mods").join(&mod_folder);
    let is_enabled = state.data.loaded_mods[mod_idx].enabled;

    let mut toggle_clicked = false;
    render_action_buttons(ui, state, &mod_path, is_enabled, &mod_folder, &mut toggle_clicked);

    if state.data.selected_mod.is_none() {
        return;
    }
    ...
    ui.horizontal(|ui| {
        ui.vertical(|ui| {
            ...
            ui.horizontal(|ui| {
                ...
                if ui.add(egui::TextEdit::singleline(&mut state.data.loaded_mods[mod_idx].metadata.author)
            ...
            ui.horizontal(|ui| {
                ...
                if ui.add(egui::TextEdit::singleline(&mut state.data.loaded_mods[mod_idx].metadata.version)
                    .desired_width(100.0)).lost_focus() {
                    metadata_changed = true;
                }
            });
            ...
            ui.horizontal(|ui| {
                ...
                if ui.add(egui::TextEdit::singleline(&mut state.data.loaded_mods[mod_idx].metadata.package)
                    .desired_width(100.0)).lost_focus() {
                    metadata_changed = true;
                }
            });
        ...
        ui.vertical(|ui| {
            ...
            if ui.add(
                egui::TextEdit::multiline(&mut state.data.loaded_mods[mod_idx].metadata.description)
                    .hint_text(desc_hint)
                    .desired_width(ui.available_width())
                    .desired_rows(5)
            ).lost_focus() {
    ...
    if metadata_changed {
        state.data.loaded_mods[mod_idx].metadata.title = state.data.loaded_mods[mod_idx].folder_name.clone();
        let _ = state.data.loaded_mods[mod_idx].metadata.save(&mod_path);
    }

    if toggle_clicked {
        for m in state.data.loaded_mods.iter_mut() {
            m.enabled = false;
        }
        if !is_enabled {
...
fn render_action_buttons(
    ...
    ui.horizontal(|ui| {
        ...
        if ui.add_sized(BTN_SIZE, export_btn).clicked() {
            state.data.export.is_open = true;
        }
