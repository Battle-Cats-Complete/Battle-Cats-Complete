    ...
    std::thread::spawn(move || {
        ...
        let mut parsed_enemies: Vec<EnemyEntry> = raw_enemies.into_par_iter().enumerate().filter_map(|(id, stats)| {
            ...
            if let (Some(parent), Some(name)) = (icon_p.parent(), icon_p.file_name().and_then(|n| n.to_str())) {
                resolved_icon = crate::common::resolver::get(parent, [name], priority).into_iter().next();
            }
            ...
            if let (Some(parent), Some(name)) = (atk_p.parent(), atk_p.file_name().and_then(|n| n.to_str()))
                && let Some(resolved_atk) = crate::common::resolver::get(parent, [name], priority).into_iter().next()
        ...
        if !crate::common::resolver::is_mod_active() {
            ...
            crate::common::io::cache::save("enemies_cache.bin", current_hash, &parsed_enemies);
        }
    });
...
pub fn scan_single(id: u32, config: &ScannerConfig) -> Option<EnemyEntry> {
    ...
    if let (Some(parent), Some(name)) = (atk_p.parent(), atk_p.file_name().and_then(|n| n.to_str())) {
        let resolved_atk = crate::common::resolver::get(parent, [name], priority).into_iter().next();
