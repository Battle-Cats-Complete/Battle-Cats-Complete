...
pub fn start_scan(config: ScannerConfig) -> Receiver<CatEntry> {
    ...
    thread::spawn(move || {
        ...
        let unitbuy_resolved = crate::common::resolver::get(cats_directory, [paths::UNIT_BUY], priority).into_iter().next();
        let unitlevel_resolved = crate::common::resolver::get(cats_directory, [paths::UNIT_LEVEL], priority).into_iter().next();
        ...
        if !crate::common::resolver::is_mod_active() {
            let current_hash = crate::common::io::cache::get_game_hash(None);
            crate::common::io::cache::save("cats_cache.bin", current_hash, &parsed_cats);
        }
    });
...
pub fn process_cat_entry(
    ...
    let resolved_stats = crate::common::resolver::get(stats_parent, [stats_file_name], priority).into_iter().next();
    ...
    for form_idx in 0..4 {
        ...
        let mut resolved_banner = crate::common::resolver::get(&dir, [banner_name.as_str()], priority).into_iter().next();
        ...
        if resolved_banner.is_none() && form_idx == 1 && egg_ids.1 != -1 {
            ...
            resolved_banner = crate::common::resolver::get(&dir, [fallback_name.as_str()], priority).into_iter().next();
        }

        let icon_stem = paths::image_stem(paths::AssetType::Icon, cat_id, form_idx, egg_ids);
        let icon_name = format!("{}.png", icon_stem);
        let mut resolved_icon = crate::common::resolver::get(&dir, [icon_name.as_str()], priority).into_iter().next();

        if resolved_icon.is_none() && form_idx == 1 && egg_ids.1 != -1 {
            let fallback_stem = format!("uni{:03}_m00", egg_ids.1);
            let fallback_name = format!("{}.png", fallback_stem);
            resolved_icon = crate::common::resolver::get(&dir, [fallback_name.as_str()], priority).into_iter().next();
        }
    ...
    for form_idx in (0..=config.preferred_form).rev() {
        if forms_existence[form_idx] {
            ...
            let banner_stem = paths::image_stem(paths::AssetType::Banner, cat_id, form_idx, egg_ids);
            let banner_name = format!("{}.png", banner_stem);
            let mut resolved_fallback = crate::common::resolver::get(&dir, [banner_name.as_str()], priority).into_iter().next();

            if resolved_fallback.is_none() && form_idx == 1 && egg_ids.1 != -1 {
                let fallback_stem = format!("udi{:03}_m00", egg_ids.1);
                let fallback_name = format!("{}.png", fallback_stem);
                resolved_fallback = crate::common::resolver::get(&dir, [fallback_name.as_str()], priority).into_iter().next();
            }
    ...
    for i in 0..4 {
        ...
        if let (Some(parent_dir), Some(anim_file_name)) = (anim_path.parent(), anim_path.file_name().and_then(|name_str| name_str.to_str()))
            && let Some(resolved) = crate::common::resolver::get(parent_dir, [anim_file_name], priority).into_iter().next()
