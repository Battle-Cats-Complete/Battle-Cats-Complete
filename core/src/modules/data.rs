...
pub struct DataConfigState {
    ...
    #[serde(skip)]
    pub import_log_content: String,
    #[serde(skip)]
    pub import_rx: Option<Receiver<String>>,
    #[serde(skip)]
    pub import_job_status: Arc<AtomicU8>,
    #[serde(skip)]
    pub import_abort_flag: Arc<AtomicBool>,
    #[serde(skip)]
    pub import_job_completed_time: Option<Instant>,
    #[serde(skip)]
    pub import_job_aborted_time: Option<Instant>,
    #[serde(skip)]
    pub import_progress_current: Arc<AtomicUsize>,
    #[serde(skip)]
    pub import_progress_maximum: Arc<AtomicUsize>,

    #[serde(skip)]
    pub export_log_content: String,
    #[serde(skip)]
    pub export_rx: Option<Receiver<String>>,
    #[serde(skip)]
    pub export_job_status: Arc<AtomicU8>,
    #[serde(skip)]
    pub export_abort_flag: Arc<AtomicBool>,
    #[serde(skip)]
    pub export_job_completed_time: Option<Instant>,
    #[serde(skip)]
    pub export_job_aborted_time: Option<Instant>,
    #[serde(skip)]
    pub export_progress_current: Arc<AtomicUsize>,
    #[serde(skip)]
    pub export_progress_maximum: Arc<AtomicUsize>,
}
...
impl DataConfigState {
    pub fn tick_threads(&mut self) -> UpdateFlags {
        let mut flags = UpdateFlags {
            import_finished_just_now: false,
            needs_repaint: false,
        };
        ...
        if let Some(time) = self.import_job_completed_time {
            if time.elapsed().as_secs() < 2 {
                ...
                self.import_job_completed_time = None;
            }
        }
        ...
        if let Some(time) = self.import_job_aborted_time {
            if time.elapsed().as_secs() < 2 {
                ...
                self.import_job_aborted_time = None;
            }
        }
        ...
        if let Some(time) = self.export_job_completed_time {
            if time.elapsed().as_secs() < 2 {
                ...
                self.export_job_completed_time = None;
            }
        }
        ...
        if let Some(time) = self.export_job_aborted_time {
            if time.elapsed().as_secs() < 2 {
                ...
                self.export_job_aborted_time = None;
            }
        }
